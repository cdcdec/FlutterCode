...
/// 网络请求封装
class HttpUtils {
  final _logger = Logger('HttpUtils');
  ...
  Future<Response> download(url, path,
          {ProgressCallback receive, CancelToken token}) =>
    ...
    try {
      ...
      if (method == GET) {
        ...
        rep = await _dio.get(url,
            options: opt, onReceiveProgress: receive, cancelToken: token);
      } else if (method == POST) {
        rep = params == null
            ? await _dio.post(url,
                options: opt,
